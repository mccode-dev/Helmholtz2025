...
[tmp,output]=unix('ls -rt | grep -v instr | grep -v \.eps | grep -v \.png');
...
for j=1:siz(1)-1
  ...
  if(length(output{j})>0)
    ...
    if j==1
      ...
      plot(data(:,1),extra(:,2)./data(:,2),marker)
    end
  end
end
